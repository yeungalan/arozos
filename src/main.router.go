	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		*/
		...
		if r.URL.Path == "/favicon.ico" || r.URL.Path == "/manifest.webmanifest" || r.URL.Path == "/robots.txt" || r.URL.Path == "/humans.txt" {
			...
			// CalDAV service discovery redirect
			http.Redirect(w, r, "/caldav/", http.StatusMovedPermanently)
		} else if len(r.URL.Path) >= len("/caldav") && r.URL.Path[:7] == "/caldav" {
			// CalDAV sub-router (handles its own Basic Auth)
			if CalDAVServer != nil {
				CalDAVServer.HandleRequest(w, r)
			} else {
				http.Error(w, "CalDAV service not available", http.StatusServiceUnavailable)
			}
